<!-- GENERATED FILE — do not edit. Source: shared/references/register-matrix.md (sha256 34b6dbbd9d8ed910). Edit the source and run `python tools/sync_shared.py`. -->
<!-- vlc-disable: TONE001 -->
...
| `thesis-vi` | | `tôi`, `chúng tôi`, `bạn`, `quý vị`, `quý khách`, `em`, `con` | Luận văn Thạc sĩ, Tiến sĩ — phong cách học thuật ngôi thứ ba hoàn toàn khách quan (`luận văn`, `đề tài`, `nghiên cứu này`) |
| `paper-ieee` | | `tôi`, `bạn`, `quý vị`, `quý khách` | Bài báo hội nghị / tạp chí IEEE — phong cách khoa học khách quan |
| `paper-q1` | | `tôi`, `bạn`, `quý vị`, `quý khách` | Bài báo tạp chí ISI/Scopus Q1 — lập luận khoa học chặt chẽ, không xưng hô phi học thuật |
